...
@Component({
  selector: 'app-kte-decharge',
  templateUrl: './kte-decharge.component.html',
  styleUrls: ['./kte-decharge.component.scss']
})
export class KteDechargeComponent implements OnInit {
  ...
  public bdLink = 'kte-decharges';
  public storageLink = 'KTE/DCG/';
  public companytag = '/KTE/';
  public doctype = '/DCG/';

  public deviationpage = '/dashboard/ktecourriersortant';
  public actualcomp = '/dashboard/ktedecharge';
  ...
  getRefComplete(lRow) {
    ...
    this.reference = 'N°' + this.lpad(lRow, 4) + this.doctype + this.getInitials(b) + this.companytag +
      this.getInitials(c) + '/' + this.getInitials(d) + '/' + this.lpad(day, 2) +
  ...
  initialize() {
    ...
    try {
      this.firesv.get(this.bdLink).forEach((actions: DocumentChangeAction<unknown>[]) => {
