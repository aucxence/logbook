...
@Component({
  selector: 'app-tm-courrier-sortant',
  templateUrl: './tm-courrier-sortant.component.html',
  styleUrls: ['./tm-courrier-sortant.component.scss']
})
export class TmCourrierSortantComponent implements OnInit {
  ...
  getRefComplete(lRow) {
    ...
    this.reference = 'N°' + this.lpad(lRow, 4) + '/CS/' + this.getInitials(b) + '/TM/' +
  ...
  initialize() {
    ...
    try {
      this.firesv.get('tm-courriers-sortants').forEach((actions: DocumentChangeAction<unknown>[]) => {
  ...
  ngOnInit(): void {
    ...
    try {
      this.sbl = this.firesv.get('tm-courriers-sortants').subscribe((actions: DocumentChangeAction<unknown>[]) => {
  ...
  submitTMCS() {
    if (this.form.valid) {
      try {
        ...
        this.firesv.add('tm-courriers-sortants', k)
          .then((response) => {
            ...
            this._toastr.success(response.code, response.message, {
              ...
              this.router.navigateByUrl('/dashboard/tmcourrierentrant', { skipLocationChange: true }).then((b) => {
                this.router.navigate(['/dashboard/tmcourriersortant']);
              });
              // this.rerender();
              // this.ngOnInit();
              ...
              // this.router.navigateByUrl("/dashboard/tmcourriersortant", { skipLocationChange: true });
  ...
  uploadTMCS($event, avant: boolean, id: string, index) {
    ...
    try {
      if (avant === true) {
        ...
        this.firesv.pushUpload('tm-courriers-sortants', 'TM/CS/', this.currentUploadbefore, avant, id)
          .then((response) => {
        ...
        this.firesv.pushUpload('tm-courriers-sortants', 'TM/CS/', this.currentUploadafter, avant, id)
          .then((response) => {
            const msg = response.message.split(' ');
