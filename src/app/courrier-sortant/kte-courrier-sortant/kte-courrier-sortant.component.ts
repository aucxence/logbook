...
@Component({
  selector: 'app-kte-courrier-sortant',
  templateUrl: './kte-courrier-sortant.component.html',
  styleUrls: ['./kte-courrier-sortant.component.scss']
})
export class KteCourrierSortantComponent implements OnInit {
  ...
  getRefComplete(lRow) {
    ...
    this.reference = 'N°' + this.lpad(lRow, 4) + '/CS/' + this.getInitials(b) + '/KTE/' +
  ...
  initialize() {
    ...
    try {
      this.firesv.get('kte-courriers-sortants').forEach((actions: DocumentChangeAction<unknown>[]) => {
  ...
  ngOnInit(): void {
    ...
    try {
      this.sbl = this.firesv.get('kte-courriers-sortants').subscribe((actions: DocumentChangeAction<unknown>[]) => {
  ...
  submitKTECS() {
    if (this.form.valid) {
      try {
        ...
        this.firesv.add('kte-courriers-sortants', k)
          .then((response) => {
            ...
            this._toastr.success(response.code, response.message, {
              ...
              this.router.navigateByUrl('/dashboard/ktecourrierentrant', { skipLocationChange: true }).then((b) => {
                this.router.navigate(['/dashboard/ktecourriersortant']);
              });
              // this.rerender();
              // this.ngOnInit();
              ...
              // this.router.navigateByUrl("/dashboard/ktecourriersortant", { skipLocationChange: true });
  ...
  uploadKTECS($event, avant: boolean, id: string, index) {
    ...
    try {
      if (avant === true) {
        ...
        this.firesv.pushUpload('kte-courriers-sortants', 'KTE/CS/', this.currentUploadbefore, avant, id)
          .then((response) => {
        ...
        this.firesv.pushUpload('kte-courriers-sortants', 'KTE/CS/', this.currentUploadafter, avant, id)
          .then((response) => {
            const msg = response.message.split(' ');
