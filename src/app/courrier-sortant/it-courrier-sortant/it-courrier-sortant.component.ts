...
@Component({
  selector: 'app-it-courrier-sortant',
  templateUrl: './it-courrier-sortant.component.html',
  styleUrls: ['./it-courrier-sortant.component.scss']
})
export class ItCourrierSortantComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  getRefComplete(lRow) {
    ...
    this.reference = 'N°' + this.lpad(lRow, 4) + '/CS/' + this.getInitials(b) + '/IT/' +
  ...
  ngOnInit(): void {
    ...
    try {
      this.sbl = this.firesv.get('it-courriers-sortants').subscribe((actions: DocumentChangeAction<unknown>[]) => {
  ...
  submitITCS() {
    if (this.form.valid) {
      try {
        ...
        this.firesv.add('it-courriers-sortants', k)
          .then((response) => {
            ...
            this._toastr.success(response.code, response.message, {
              ...
              this.router.navigateByUrl('/dashboard/itcourrierentrant', { skipLocationChange: true }).then((b) => {
                this.router.navigate(['/dashboard/itcourriersortant']);
              });
              // this.rerender();
              // this.ngOnInit();
              ...
              // this.router.navigateByUrl("/dashboard/itcourriersortant", { skipLocationChange: true });
  ...
  uploadITCS($event, avant: boolean, id: string, index) {
    ...
    try {
      if (avant === true) {
        ...
        this.firesv.pushUpload('it-courriers-sortants', 'IT/CS/', this.currentUploadbefore, avant, id)
          .then((response) => {
        ...
        this.firesv.pushUpload('it-courriers-sortants', 'IT/CS/', this.currentUploadafter, avant, id)
          .then((response) => {
            const msg = response.message.split(' ');
