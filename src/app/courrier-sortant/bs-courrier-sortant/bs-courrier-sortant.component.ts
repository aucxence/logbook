...
@Component({
  selector: 'app-bs-courrier-sortant',
  templateUrl: './bs-courrier-sortant.component.html',
  styleUrls: ['./bs-courrier-sortant.component.scss']
})
export class BsCourrierSortantComponent implements OnInit {
  ...
  getRefComplete(lRow) {
    ...
    this.reference = 'N°' + this.lpad(lRow, 4) + '/CS/' + this.getInitials(b) + '/BS/' +
  ...
  initialize() {
    ...
    try {
      this.firesv.get('bs-courriers-sortants').forEach((actions: DocumentChangeAction<unknown>[]) => {
  ...
  ngOnInit(): void {
    ...
    try {
      this.sbl = this.firesv.get('bs-courriers-sortants').subscribe((actions: DocumentChangeAction<unknown>[]) => {
  ...
  submitBSCS() {
    if (this.form.valid) {
      try {
        ...
        this.firesv.add('bs-courriers-sortants', k)
          .then((response) => {
            ...
            this._toastr.success(response.code, response.message, {
              ...
              this.router.navigateByUrl('/dashboard/bscourrierentrant', { skipLocationChange: true }).then((b) => {
                this.router.navigate(['/dashboard/bscourriersortant']);
              });
              // this.rerender();
              // this.ngOnInit();
  ...
  uploadBSCS($event, avant: boolean, id: string, index) {
    ...
    try {
      if (avant === true) {
        ...
        this.firesv.pushUpload('bs-courriers-sortants', 'BS/CS/', this.currentUploadbefore, avant, id)
          .then((response) => {
        ...
        this.firesv.pushUpload('bs-courriers-sortants', 'BS/CS/', this.currentUploadafter, avant, id)
          .then((response) => {
            const msg = response.message.split(' ');
