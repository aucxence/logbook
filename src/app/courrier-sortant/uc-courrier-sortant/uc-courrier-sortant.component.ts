...
@Component({
  selector: 'app-uc-courrier-sortant',
  templateUrl: './uc-courrier-sortant.component.html',
  styleUrls: ['./uc-courrier-sortant.component.scss']
})
export class UcCourrierSortantComponent implements OnInit {
  ...
  getRefComplete(lRow) {
    ...
    this.reference = 'N°' + this.lpad(lRow, 4) + '/CS/' + this.getInitials(b) + '/UCASH/' +
  ...
  initialize() {
    ...
    try {
      this.firesv.get('uc-courriers-sortants').forEach((actions: DocumentChangeAction<unknown>[]) => {
  ...
  ngOnInit(): void {
    ...
    try {
      this.sbl = this.firesv.get('uc-courriers-sortants').subscribe((actions: DocumentChangeAction<unknown>[]) => {
  ...
  submitUCCS() {
    if (this.form.valid) {
      try {
        ...
        this.firesv.add('uc-courriers-sortants', k)
          .then((response) => {
            ...
            this._toastr.success(response.code, response.message, {
              ...
              this.router.navigateByUrl('/dashboard/uccourrierentrant', { skipLocationChange: true }).then((b) => {
                this.router.navigate(['/dashboard/uccourriersortant']);
              });
              // this.rerender();
              // this.ngOnInit();
              ...
              // this.router.navigateByUrl("/dashboard/uccourriersortant", { skipLocationChange: true });
  ...
  uploadUCCS($event, avant: boolean, id: string, index) {
    ...
    try {
      if (avant === true) {
        ...
        this.firesv.pushUpload('uc-courriers-sortants', 'UCASH/CS/', this.currentUploadbefore, avant, id)
          .then((response) => {
        ...
        this.firesv.pushUpload('uc-courriers-sortants', 'UCASH/CS/', this.currentUploadafter, avant, id)
          .then((response) => {
            const msg = response.message.split(' ');
