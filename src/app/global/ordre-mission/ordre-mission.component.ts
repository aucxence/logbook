...
import { OrdreMission } from 'src/app/models/ordre-mission';
declare let $;

@Component({
  selector: 'app-ordre-mission',
  templateUrl: './ordre-mission.component.html',
  styleUrls: ['./ordre-mission.component.scss']
})
export class OrdreMissionComponent implements OnInit {
  ...
  public companystate = {
    type: 'un Ordre de Mission',
    typepluriel: 'des Ordres de mission',
    typeunique: 'l\'ordre de mission',
    doctype: '/OM',
    // companyname: 'Instant Transfer',
    // bdLink: 'it-ordres-mission',
    // storageLink: 'IT/OM/',
    // companytag: '/IT/',
    // deviationpage: '/dashboard/itcourriersortant',
    // actualcomp: '/dashboard/ordremission'
  }
  ...
  public state = {
    [this.companies.biostar]: {
      ...this.companystate,
      companyname: 'BioSTAR',
      bdLink: 'bs-ordres-mission',
      storageLink: 'BS/OM/',
      companytag: '/BS/',
      deviationpage: '/dashboard/bscourriersortant',
      actualcomp: '/dashboard/ordremission/' + this.companies.biostar
    },
    [this.companies.technomed]: {
      ...this.companystate,
      companyname: 'Technologie Médicale',
      bdLink: 'tm-ordres-mission',
      storageLink: 'TM/OM/',
      companytag: '/TMCO/',
      deviationpage: '/dashboard/tmcourriersortant',
      actualcomp: '/dashboard/ordremission/' + this.companies.technomed
    },
    [this.companies.kamsu]: {
      ...this.companystate,
      companyname: 'Kamsu Tankeu Eugénie',
      bdLink: 'kte-ordres-mission',
      storageLink: 'KTE/OM/',
      companytag: '/KTE/',
      deviationpage: '/dashboard/ktecourriersortant',
      actualcomp: '/dashboard/ordremission/' + this.companies.kamsu
    },
    [this.companies.unicash]: {
      ...this.companystate,
      companyname: 'Universal Cash',
      bdLink: 'uc-ordres-mission',
      storageLink: 'UCASH/OM/',
      companytag: '/UCASH/',
      deviationpage: '/dashboard/uccourriersortant',
      actualcomp: '/dashboard/ordremission/' + this.companies.unicash
    },
    [this.companies.muffa]: {
      ...this.companystate,
      companyname: 'Muffa MC2 Kye Ossi',
      bdLink: 'mc2-ordres-mission',
      storageLink: 'MC2/OM/',
      companytag: '/MMKO/',
      deviationpage: '/dashboard/mc2courriersortant',
      actualcomp: '/dashboard/ordremission/' + this.companies.muffa
    },
    [this.companies.itransfer]: {
      ...
      companytag: '/IT/',
      deviationpage: '/dashboard/itcourriersortant',
      actualcomp: '/dashboard/ordremission/' + this.companies.itransfer
    }
  };
  ...
  private setupForm() {
    this.form = this.formBuilder.group({
      datereception: [new Date().getSeconds(), [Validators.required]],
      ...
      ville: [null, [Validators.required, Validators.minLength(3), Validators.maxLength(100)]],
      villedepart: [null, [Validators.required, Validators.minLength(3), Validators.maxLength(100)]],
  ...
  getRefComplete(lRow) {
    ...
    this.reference = 'N°' + this.lpad(lRow, 4) + this.relevantstate.doctype + this.relevantstate.companytag + destinataire +
      '/' + this.villedepart + '/' + this.ville + '/' + this.lpad(dayr, 2) + '' + this.lpad(monthr, 2) + '/' + yearr;
      ...
      try {
        this.sbl = this.firesv.get(this.relevantstate.bdLink).subscribe((actions: DocumentChangeAction<unknown>[]) => {
          ...
          actions.forEach((action) => {
            // console.log('--------------------------------');
            const data = action.payload.doc.data() as OrdreMission;
            ...
            this.rows.push({
              ...
              objet: docdata.objet,
              ville: docdata.ville,
              villedepart: docdata.villedepart,
    ...
    if (this.form.valid) {
      try {
        const k = {
          ...
          villedepart: this.form.value.villedepart,
